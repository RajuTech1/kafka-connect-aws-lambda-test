...
public class AwsLambdaSinkTask extends SinkTask {
  ...
  public void put(Collection<SinkRecord> collection) {
      ...
      List<Function<String, InvokeRequest>> invokeRequestWithPayloadList = connectorConfig.getInvokeRequestWithPayload(); 
      for(Function<String, InvokeRequest> invokeRequestWithPayload : invokeRequestWithPayloadList){
          loggingWrapper(collection.stream()
            .map(connectorConfig.getPayloadConverter())
            .map(invokeRequestWithPayload))
