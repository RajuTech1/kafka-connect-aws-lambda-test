...
public class AwsLambdaSinkConnectorConfig extends AbstractConfig {
  ...
  private final List<InvokeRequest> invokeRequestList = new ArrayList<InvokeRequest>();
  ...
  private AwsLambdaSinkConnectorConfig(ConfigDef config, Map<String, String> parsedConfig) {
    ...
    String[] functionNames = getAwsFunctionNames();
    for(String funtionName : functionNames){
        InvokeRequest invokeRequest = new InvokeRequest()
          .withFunctionName(funtionName)
          .withInvocationType(getAwsLambdaInvocationType());

        invokeRequestList.add(invokeRequest);
    }
  }
  ...
  public String[] getAwsFunctionNames() {
    String functionNames = this.getString(FUNCTION_NAME_CONFIG);
    return functionNames.split(",");
  }
  ...
  public List<Function<String, InvokeRequest>> getInvokeRequestWithPayload() {
    
    List<Function<String, InvokeRequest>> invokeRequestFunctionList = new ArrayList<Function<String, InvokeRequest>>();

    for(InvokeRequest invokeRequest : invokeRequestList){
        Function<String, InvokeRequest> invokeRequestFunction = invokeRequest::withPayload;
        invokeRequestFunctionList.add(invokeRequestFunction);
    }

    return invokeRequestFunctionList;
  }
